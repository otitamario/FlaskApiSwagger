...
class AwesomePostResponseSchema(Schema):
    message = fields.Str(default='Success')
    api_type = fields.Str()
...
class AwesomeAPI(MethodResource, Resource):
    ...
    def get(self):
        ...
        '''
        return {'message': 'My First GET Awesome API'}

    @doc(description='My First POST Awesome API.', tags=['Awesome'])
    @use_kwargs(AwesomeRequestSchema, location=('json'))
    @marshal_with(AwesomePostResponseSchema)  # marshalling
    def post(self, **kwargs):
        '''
        Get method represents a POST API method
        '''
        return {"message": "My First POST Awesome API", "api_type":kwargs["api_type"]}
